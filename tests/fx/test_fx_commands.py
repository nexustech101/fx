...
def test_fx_install_builds_editable_install_commands(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("fx.plugins.runtime.run_checked", _fake_run_checked)
    monkeypatch.setattr("fx.plugins.runtime.progress_steps", _fake_progress_steps)
    ...
    monkeypatch.setattr("fx.plugins.runtime.ensure_venv_python", _fake_venv_python)
...
def test_fx_update_builds_source_specific_commands_and_validates_flags(
    ...
    assert calls[0] == [sys.executable, "-m", "pip", "install", "--upgrade", "registers"]
    assert progress_calls[0][0] == 3
    ...
    run(
        [
            ...
            "git",
            "--repo",
            "https://github.com/nexustech101/registers.git",
            "--ref",
            "main",
        ],
        print_result=False,
    )
    assert calls[1][-1] == "git+https://github.com/nexustech101/registers.git@main#egg=registers"
    assert progress_calls[1][0] == 3
...
def test_fx_pull_syncs_plugins_and_updates_registry(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("fx.plugins.runtime.clone_repo", _fake_clone_repo)
...
def test_fx_cron_jobs_and_trigger(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "attempts=5" in jobs
    assert "retry=fixed" in jobs
...
def test_fx_cron_status_surfaces_dead_letter_and_failure_counters(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.chdir(tmp_path)
...
def test_fx_cron_start_and_stop_background_mode(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("fx.plugins.cron._sync_cron_jobs", lambda _root: ("app", 2, 2))
    monkeypatch.setattr("fx.plugins.cron._spawn_cron_daemon", lambda **_kwargs: 43210)
    ...
    monkeypatch.setattr("fx.plugins.cron.os.kill", _fake_kill)
