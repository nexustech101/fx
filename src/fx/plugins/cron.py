...
from registers.cron.runtime import build_event_payload, run_daemon, sync_project_jobs
...
def cron_manage(
    ...
    try:
        ...
        if normalized == "status":
            ...
            dead_letter_events = cron_event_registry(root_path).count(
            ...
            return render_runtime_summary(
                ...
                fields=[
                    ...
                    ("Dead-letter events", dead_letter_events),
                    ("Runs", total_runs),
                ],
            )
        ...
        if normalized == "jobs":
            ...
            for row in rows:
                retry_bits = [row.retry_policy]
                if row.retry_policy != "none":
                    retry_bits.append(f"attempts={row.retry_max_attempts}")
                    retry_bits.append(f"backoff={row.retry_backoff_seconds}s")
                    if row.retry_max_backoff_seconds > 0:
                        retry_bits.append(f"max_backoff={row.retry_max_backoff_seconds}s")
                    if row.retry_jitter_seconds > 0:
                        retry_bits.append(f"jitter={row.retry_jitter_seconds}s")
                retry_label = ", ".join(retry_bits)
                lines.append(
                    f"  {row.name} ({row.trigger_kind}, target={row.target}, enabled={row.enabled}, retry={retry_label})"
                )
            return "\n".join(lines)
