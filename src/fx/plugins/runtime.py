...
def run_project(
    ...
    try:
        ...
        has_src_package = bool(package_dir is not None and package_dir.parent == src_root)
        if has_src_package:
            cwd = src_root

        has_cli_layout = bool(package_dir is not None and (package_dir / "todo.py").exists()) or (root_path / "app.py").exists()
        has_db_layout = bool(package_dir is not None and (package_dir / "api.py").exists()) or (root_path / "models.py").exists()
        if not project_type:
...
@register(name="update", description="Update the registers package from selected source")
@option("--update")
@argument("root", type=str, default=".", help="Project root path")
...
@argument("package", type=str, default="registers", help="Package name/egg name to upgrade")
def update_project(
    root: str = ".",
